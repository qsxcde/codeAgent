...
def _load_skills(cfg: Any = None) -> tuple[list[Any], list[Any]]:
    ...
    from codeagent.app.skills.prompt import load_skills
...
def _build_system_prompt(cfg: Any = None, skills: list[Any] | None = None) -> str:
    ...
    base = build_system_prompt(
        read_base_prompt(), load_agents_files(_workspace(cfg), CONFIG_DIR)
    )
    if skills is None:
        skills, _ = _load_skills(cfg)
    from codeagent.app.skills.runtime import build_bootstrap_prompt
    from codeagent.app.skills.prompt import build_skills_prompt
...
def agents_sources(cfg: Any = None) -> list[str]:
    """返回本次装配加载的上下文文件来源列表。"""
    from codeagent.app.context.agents import load_agents_files
    from codeagent.app.config import CONFIG_DIR

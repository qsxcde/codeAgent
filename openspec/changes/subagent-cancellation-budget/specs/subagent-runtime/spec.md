...
- **THEN** 系统创建一个新的委派请求，使用当前父 run_id，保留所选 profile，记录可选 budget，默认有效深度为 1，并等待对应子运行结果
...
- **THEN** `delegate` 返回非错误 ToolResult，内容为不超过预算的有限摘要，details 保留委派标识、子运行标识和 `completed` 状态，父 Agent 可据此继续请求模型
...
- **THEN** 只有对应的子 Session 收到取消请求，runner 等待其结束并返回 `cancelled` 与 `parent_cancelled`，或在清理不确定时附带 cleanup_uncertain

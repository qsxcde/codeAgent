...
系统 SHALL 通过稳定且唯一的规范路径提供 AI 层能力：模型消息、响应、工具定义、流事件和相关协议从 `codeagent.ai.model` 导入，SSE 解析器从 `codeagent.ai.transport.sse` 导入；provider、transport 和 catalog 的现有规范子包 SHALL 继续可用。

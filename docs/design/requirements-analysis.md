...
| 测试基线(附录 B) | 336(2026-08-14) | **666 项全绿**(2026-08-22 实测；2026-08-21 综合审计 12 条缺陷已全部修复闭环,见审计与 v0.3 §6.5) |
...
| NFR-M2 | 测试覆盖 | 核心编排层 100% 离线可测,总体覆盖率 ≥ 80% | `FakeClient` 注入;2026-08-22 实测 666 项全绿 | ✅(覆盖率测量工具待补,见审计) |
...
| NFR-E4 | 形态扩展 | 事件流多平台订阅 | CLI / Web / 测试 / CI 均可订阅;Web/HTTP 入口(F-27)已移出 v0.3(2026-08-22,E12)——契约已被 CLI/TUI/测试/CI 订阅满足,Web 仅潜在消费方 |
...
1. **测试全绿**:`uv run pytest` 全量通过（2026-08-22 实测 666 项全绿）;核心编排层零网络、零密钥(`FakeClient`)可跑通全量;

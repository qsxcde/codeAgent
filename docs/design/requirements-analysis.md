...
> 更新日期: 2026-08-24(§0.1 状态勘误复核;正文中的历史需求分析数据保留为历史快照)
> 编制口径: 以 `docs/design/` 下四份文档(需求分析报告 v0.1、架构设计、功能表 GAP 分析、编排自研蓝图)为需求来源综合而成,并对照 **2026-08-24 当前代码树**校准(校准明细见 §0.1 与附录 B)。**架构现状以 §0.1 勘误 + architecture.md(v0.3.0) 为准**。
...
## 0.1 状态勘误(2026-08-24,自研编排与 v0.3.0 已落地)
...
| 测试基线(附录 B) | 336(2026-08-14) | **666 项收集，665 passed / 1 skipped，无失败**(2026-08-24 复核；唯一跳过项是 Windows 无符号链接权限；2026-08-21 综合审计 12 条缺陷已全部修复闭环,见审计复核与 v0.3 §6.5) |
...
| [architecture.md](architecture.md) | 架构设计文档 v0.3.0(2026-08-24) |
| [feature-gap-analysis.md](feature-gap-analysis.md) | 功能表全面分析 / 竞品对标 GAP(2026-08-10) |
| [self-built-orchestration-blueprint.md](self-built-orchestration-blueprint.md) | 编排引擎自研决策与收益记录(第二步已落地,2026-08-24 校准) |
| README.md / pyproject.toml / src/codeagent/ | 项目现状(2026-08-24 复核) |
...
| F-28 | Windows 验证 | ✅ 已闭环:bash 探测链适配 + fix-bash-test-assertions 断言修复(2026-08-13)+ WSL 转发器探测修复(2026-08-14);2026-08-24 复核全量 **666 收集 / 665 通过 / 1 跳过**(无失败,跳过原因为符号链接权限) |
...
| NFR-C3 | 平台 | macOS / Linux 优先 | ✅ 已闭环:bash 含 Git for Windows / WSL 探测链(2026-08-14 修复 WSL 转发器命中;2026-08-24 Windows 全量 666 收集 / 665 通过 / 1 跳过,无失败) |
...
| R3 | 技术 | Windows 平台 bash 行为差异(路径显示、PIPESTATUS) | 高 | 低~中 | ✅ 已闭环;2026-08-24 全量 666 收集 / 665 通过 / 1 跳过,无失败 |
...
1. **测试无失败**:`uv run pytest` 全量通过（2026-08-24 复核 666 项收集 / 665 通过 / 1 跳过,唯一跳过为 Windows 符号链接权限）;核心编排层零网络、零密钥(`FakeClient`)可跑通全量;
...
| NFR-C1~C4 | 兼容性 | ✅ Windows bash 测试已修复(标记文件法,2026-08-13 fix-bash-test-assertions);2026-08-24 全量回归通过 | R §4.7; C |

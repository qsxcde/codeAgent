...
async def test_fake_returns_fixed_response():
    model = FakeClient(response="你好")
    resp = await (model.generate(_msgs("你是谁")))
    assert resp.content == "你好"


async def test_fake_returns_scripted_responses():
    model = FakeClient(response="兜底", responses=["第一轮", "第二轮"])
    assert (await model.generate(_msgs("1"))).content == "第一轮"
    assert (await model.generate(_msgs("2"))).content == "第二轮"
    # 耗尽后回落到 response
    assert (await model.generate(_msgs("3"))).content == "兜底"


async def test_fake_can_emit_tool_calls():
    model = FakeClient(
        tool_calls=[
            {
                "name": "read_file",
    ...
    resp = await (model.generate(_msgs("读文件")))
